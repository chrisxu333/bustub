...
void ExtendibleHashTable<K, V>::Insert(const K &key, const V &value) {
  size_t bucket_no = IndexOf(key);
  int status = dir_[bucket_no]->Insert(key, value);
  if(!status) {
...
auto ExtendibleHashTable<K, V>::RedistributeBucket(size_t bucket_no) -> void {
  ...
  if(local_depth > global_depth_) Grow();
  int other_idx = bucket_no^(1<<(local_depth-1)); /* find pairing index */
  dir_[other_idx].reset();
  dir_[other_idx] = std::make_shared<Bucket>(bucket_size_, local_depth);  /* change directory to points to newly splitted bucket*/
